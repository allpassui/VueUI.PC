...
; !function (window, undefined) {
		...
		that.vessel(conType, function (html, titleHTML) {
			...
			conType ? function () {
				(config.type == 2 || config.type == 4) ? function () {
					if (config.type == 4 && $(".laybox").length > 0) {
						$(".laybox").append(html[1]);
					}
					else {
						$('body').append(html[1]);
					}
	...
	Class.pt.tips = function () {
		...
		if ($(".laybox").length > 0) {
			var stuBox = {
				left: $(".laybox").offset().left,
				top: $(".laybox").offset().top,
				scrollTop: $(".laybox").scrollTop()
			}
			//辨别tips的方位
			goal.where = [function () { //上        
				...
				goal.tipLeft = goal.left + goal.width + 20 - stuBox.left;
				goal.tipTop = goal.top - stuBox.top + stuBox.scrollTop;
		...
		$(config.move).on('mousedown', function (M) {
			...
			if ($(this).attr('move') === 'ok') {
				...
				if (!$('#layui-layer-moves')[0]) {
					$('body').append('<div id="layui-layer-moves" class="layui-layer-moves" style="left:' + xx + 'px; top:' + yy + 'px; width:' + ww + 'px; height:' + hh + 'px; z-index:667"></div>');
				}
